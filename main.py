...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Привет, друг! Хочешь начать заниматься или ты уже на пути к успеху?", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def handle_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "studying":
        ...
        if cursor.fetchone():
            await query.edit_message_text("Ты успешно зарегистрирован!")
        else:
            await query.edit_message_text("Пока тебя нет в системе. Спроси репетитора!")
    elif query.data == "courses":
        await query.edit_message_text("Курсы: Математика, Подготовка к ОГЭ, ВПР и т.д. Пиши 'math+' для записи.")
